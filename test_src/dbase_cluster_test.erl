...
start()->
  ...
  %  ?assertEqual(  {ok,[Dbase0,Dbase1,Dbase2]},setup()),
    ...
    ?debugMsg("Start three"),
    ?assertEqual(ok,three(DbaseList)),
    ?debugMsg("stop three"),

    ?debugMsg("Start restart"),
    ?assertEqual(ok,restart(DbaseList)),
    ?debugMsg("stop restart"),
...
%% Returns: non
%% --------------------------------------------------------------------
restart([Dbase0,Dbase1,Dbase2])->
    ?assertMatch([{test_lock2,20}],rpc:call(Dbase0,db_lock,read_all_info,[],2000)),
    ?assertMatch([{test_lock2,20}],rpc:call(Dbase1,db_lock,read_all_info,[],2000)),
    ?assertMatch([{test_lock2,20}],rpc:call(Dbase2,db_lock,read_all_info,[],2000)),

    ?assertMatch(ok,slave:stop(Dbase0)),

    ?assertMatch({badrpc,_},rpc:call(Dbase0,db_lock,read_all_info,[],2000)),
    ?assertMatch([{test_lock2,20}],rpc:call(Dbase1,db_lock,read_all_info,[],2000)),
    ?assertMatch([{test_lock2,20}],rpc:call(Dbase2,db_lock,read_all_info,[],2000)),
...
%% Returns: non
%% --------------------------------------------------------------------
three([Dbase0,Dbase1,Dbase2])->
    ?assertMatch([{"joq62","20Qazxsw20"}],rpc:call(Dbase0,db_passwd,read_all,[],2000)),
    ?assertMatch([{"joq62","20Qazxsw20"}],rpc:call(Dbase1,db_passwd,read_all,[],2000)),
    ?assertMatch({badrpc,_},rpc:call(Dbase2,db_passwd,read_all,[],2000)),
    
    AllDbaseEnvs=application:get_all_env(dbase),

    % All slavesw have acces to the code in this test , not the case in real system
    ?assertMatch([ok,ok,ok,ok,ok,ok,ok],[rpc:call(Dbase2,application,set_env,[dbase,Par,Val])||{Par,Val}<-AllDbaseEnvs]),
    sys_log:msg(misc_oam:masters(),log,
		["Dbase0 =", Dbase2],
		?MODULE,?LINE),
    ?assertMatch(ok,rpc:call(Dbase2,application,start,[dbase],5*5000)),

     %% Test 
    AppSpecs=rpc:call(Dbase2,db_app_spec,read_all,[],2000),
    AppSpecsId=[AppId||{AppId,_AppVsn,_Type,_Directives,_AppEnvs,_Services}<-AppSpecs],

    ServiceSpecs=rpc:call(Dbase2,db_service_def,read_all,[],2000),  
    ServicesSpecsId=[SpecId||{SpecId,_ServiceId,_ServiceVsn,_StartCmd,_GitPath}<-ServiceSpecs],
    PassWd=rpc:call(Dbase2,db_passwd,read_all,[],2000),
    Servers=rpc:call(Dbase2,db_server,read_all,[],2000),
  ...
  ?assertMatch(["multi_100.service_spec",
    ...
    ?assertMatch([{"joq62","20Qazxsw20"}],rpc:call(Dbase0,db_passwd,read_all,[],2000)),
    ?assertMatch([{"joq62","20Qazxsw20"}],rpc:call(Dbase1,db_passwd,read_all,[],2000)),
    ?assertMatch([{"joq62","20Qazxsw20"}],rpc:call(Dbase2,db_passwd,read_all,[],2000)),

    %Test distribution

    ?assertMatch({atomic,ok},rpc:call(Dbase0,db_lock,create,[test_lock2,20],2000)),
    ?assertMatch([{test_lock2,20}],rpc:call(Dbase0,db_lock,read_all_info,[],2000)),
    ?assertMatch([{test_lock2,20}],rpc:call(Dbase1,db_lock,read_all_info,[],2000)),
    ?assertMatch([{test_lock2,20}],rpc:call(Dbase2,db_lock,read_all_info,[],2000)),
